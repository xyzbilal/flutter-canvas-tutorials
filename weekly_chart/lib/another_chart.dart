...
class LineData {
  ...
  Map<String, double> createCumulativeData(List<double> values) {
    ...
    values.forEach((element) {
     // sum += element;
      data.putIfAbsent(c.toString(), () => element);
      c++;
    });
    return data;
  }
...
class LineChart extends StatelessWidget {
  ...
  final List<LineData> data = [
    LineData("Hamilton", Colors.teal.shade700, hamilton),
    LineData("Bottas", Colors.teal.shade300, bottas),
    LineData("Verstappen", Colors.blue.shade900, max)
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return TweenAnimationBuilder(
        ...
        builder: (BuildContext context, double percentage, Widget? child) {
          return CustomPaint(
            painter:
                LineChartPainter(percentage, data, "BLAH",mx),
...
class LineChartPainter extends CustomPainter {
  ...
  LineChartPainter(this.percentage, this.lineData, this.title,this.maxValue) {
  ...
  void drawLines(Size size, Canvas canvas) {
   ...
   // lineData.forEach((element) {
    final element = lineData.first;
      ...
      path.moveTo(entryToPoint(points.first, size).dx,entryToPoint(points.first, size).dy);
      ...
      for (int i = 0; i < (points.length - 1); i++) {
        ...
        if (percentageOfLine > 0) {
          ...
          var nextPoint =  Offset(
            ...
            final w = (goalPoint.dx - firstPoint.dx) / 3 / 2;
           print(w);

            path.cubicTo(firstPoint.dx+w,firstPoint.dy,
             firstPoint.dx+2*w,firstPoint.dy,
             goalPoint.dx,goalPoint.dy
          ...
          canvas.drawPath(path,getLinePaint(element) );

         // drawLine( entryToPoint(points[i], size), nextPoint, getLinePaint(element));
  ...
  void addHorizontalLinesAndSizes(Size size, Canvas canvas) {
    for (int i = 1; i <= 8; i++) {
      double y = chartHeight(size) - chartHeight(size) * (i / 8) + marginTop;
      TextPainter tp = createText((maxValue / 8 * i).round().toString(), 1);
      tp.paint(canvas,
